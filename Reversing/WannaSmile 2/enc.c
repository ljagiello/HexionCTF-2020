...
void file_encrypt_decrypt(cipher_params_t *params, FILE *ifp, FILE *ofp){
    ...
    if(!EVP_CipherInit_ex(ctx, NULL, NULL, params->key, NULL, params->encrypt)){
        fprintf(stderr, "ERROR: EVP_CipherInit_ex failed. OpenSSL error: %s\n",
                ERR_error_string(ERR_get_error(), NULL));
...
int main(){
  ...
  params->key = "jmgcgmseonnehdkwpapcygwyabpwqvob"; //flag2.txt
  params->iv = NULL;
  params->encrypt = 0;
  params->cipher_type = EVP_aes_256_cbc();

  /* Open the encrypted file for reading in binary ("rb" mode) */
  f_input = fopen("flag2.txt.enc", "rb");
  if (!f_input) {
